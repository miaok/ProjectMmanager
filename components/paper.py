...
def paper_management():
    ...
    with st.expander("添加/编辑论文信息", expanded=False):
        # 获取所有论文信息用于编辑
        conn = get_connection()
        ...
        with st.form("paper_form"):
            # 如果是编辑模式，需要选择论文
            edit_mode = st.checkbox("编辑已有论文")
            
            if edit_mode and not papers_df.empty:
                paper_id = st.selectbox("选择要编辑的论文", papers_df['id'].tolist(), 
