...
def patent_management():
    ...
    with st.expander("添加/编辑专利信息", expanded=False):
        # 获取所有专利信息用于编辑
        conn = get_connection()
        ...
        with st.form("patent_form"):
            # 如果是编辑模式，需要选择专利
            edit_mode = st.checkbox("编辑已有专利")
            
            if edit_mode and not patents_df.empty:
                patent_id = st.selectbox("选择要编辑的专利", patents_df['id'].tolist(), 
                    ...
                    try:
                        ...
                        if edit_mode and patent_id:
                            ...
                            ''', (name, patent_type, application_date_str, grant_date_str, 
                                 owner_id, participants_str, company, patent_number, certificate, patent_id))
                            st.success(f"已更新专利 {name} 的信息")
                        else:
                            # 新增记录
                            cursor.execute('''
